...
const sortedResponsiveOptions = computed(() => {
  return [...props.responsiveOptions].sort(
    (a, b) => parseBreakpoint(b.breakpoint) - parseBreakpoint(a.breakpoint),
  )
})
...
const isItemVisible = index => {
  return (
    ...
    index < currentIndex.value + itemsPerView.value
  )
}
...
const updateResponsiveOptions = () => {
  ...
  if (!matched && sortedResponsiveOptions.value.length > 0) {
    const smallestOption =
      sortedResponsiveOptions.value[sortedResponsiveOptions.value.length - 1]
    ...
    <div class="w-full overflow-hidden">
      ...
      >
        ...
        <div
          ...
          :key="index"
          class="flex-shrink-0 px-2 transition-all duration-300"
