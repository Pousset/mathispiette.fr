...
const props = defineProps({
  sections: {
    ...
    required: true,
    validator: value =>
      value.every(section => 'path' in section && 'name' in section),
  },
  currentSection: {
    type: String,
    required: true,
...
const getDotClasses = sectionPath => {
  ...
  return {
    ...
    'hover:bg-primary-400 hover:w-3 hover:h-3':
      !isTouchDevice.value && !isActive && !isHovered,
  }
}
...
<template>
  <nav
    id="section-nav"
    class="fixed right-6 top-1/2 transform -translate-y-1/2 z-50"
    role="navigation"
      ...
      >
        <div class="w-4 h-4 flex items-center justify-center">
          <button
            ...
            @mouseenter="handleMouseEnter(section.path)"
            @mouseleave="handleMouseLeave"
            :class="getDotClasses(section.path)"
        ...
        >
          <div
            v-if="
              hoveredDot === section.path ||
              (currentSection === section.path && showLabel)
            "
